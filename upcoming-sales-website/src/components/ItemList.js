            ...
            <ul className={styles.itemList}>
                {sortedKeys.map((key) => (
                    <li key={key} className={styles.item}>
                        ...
                        <p>Price: {formatNumber(items[key].price)}{key.substring(0, 3) === '870' ? ' Mesos' : ' NX'} {items[key].discount == 1 ? `(was ${formatNumber(items[key].originalPrice)}${key.substring(0, 3) === '870' ? ' Mesos' : ' NX'})` : ''}</p>
